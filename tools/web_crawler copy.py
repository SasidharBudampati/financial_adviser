...
class TakeScreenshot(Tool, WebCrawlTool):
    async def run(self, tool_context: ToolContext) -> dict:
    ...
    name: ClassVar[str]= "take_sreenshot"
    description : ClassVar[str]= "Takes a screenshot and saves it with the given filename. called 'load artifacts' after to load the image" 
    ...
    def _create_emitter(self) -> Emitter:
        return Emitter.root().child(
            namespace=["tool", "take_sreenshot"],
            creator=self,
        )

class ClickAtCoordinates(Tool, WebCrawlTool):
...
class FindElementWithText(Tool, WebCrawlTool):
...
class ClickElementWithText(Tool, WebCrawlTool):
    def run(self, text: str) -> str:
...
class EnterTextIntoElement(Tool, WebCrawlTool):
...
class ScrollDownScreen(Tool, WebCrawlTool):
    def run(self) -> str:
...
class GetPageSource(Tool, WebCrawlTool):
    def run(self) -> str:
        LIMIT = 1000000
...
class AnalyzeWebpagAndDetermineAction(Tool, WebCrawlTool):
    def run(self,
